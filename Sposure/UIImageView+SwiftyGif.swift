...
public extension UIImageView {
    ...
    /**
     Check if the image has hit the end of its loop
     - Returns wether this gif has any more loops
    */
    public func hasFinishedLooping() -> Bool {
    ...
    public func updateCache() {
        if (self.animationManager as SwiftyGifManager).hasCache(self) && !self.haveCache {
            prepareCache()
            self.haveCache = true
        } else if !(self.animationManager as SwiftyGifManager).hasCache(self) && self.haveCache {
     ...
     */
    public func updateCurrentImage(_ onLoopEnd : @escaping ()->Void = {}) {
        ...
        if self.displaying{
            ...
            if isDiscarded(self) {
                (self.animationManager as SwiftyGifManager).deleteImageView(self)
            }
        }
    }
